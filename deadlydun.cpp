/*
 * Deadly dungeons a tactical action game - main entry point
 * 
 * This game uses a subset of GLESv2 and EGL
 ...
 * Compilation:
 *   - Will use make. Now: g++ deadlydun.cpp --std=c++14 -o deadlydun.out -lglut -lGLESv2 -lm
...
static void printGlError(std::string where) {
	...
	if(err != GL_NO_ERROR) {
		DD_LOGE((where + " - glError: 0x%x").c_str(), err);
	}
}
...
static void handleMouseGlutEvents(int button, int state, int x, int y) {
	DD_LOGI("mouse event(%d,%d,x:%d,y:%d)", button, state, x, y);
}
...
int main(int argc, char *argv[]) {
	...
	glutCreateWindow("Deadly Dungeons " DD_VERSION);
